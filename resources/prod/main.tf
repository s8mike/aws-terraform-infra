...
#
# Modules are identical to dev.
# Only variable values differ — larger compute,
# stricter security, higher scaling limits.
#
# Production readiness checklist before first deploy:
#   ✅ Create S3 backend bucket manually
#   ✅ Create ECR repository for prod
...
#   ⏳ Move ECS tasks to private subnets + NAT Gateway
...
# ─────────────────────────────────────────────────────────────
output "alb_dns_name" {
  description = "ALB DNS name — open in browser"
